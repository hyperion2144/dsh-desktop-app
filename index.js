...
const CONTENT = [
  ...
  '- 启动：TcpStream 探测桌面壳专用端口 127.0.0.1:3081（DSH_DESKTOP_PORT 可覆盖）——',
  '  空闲 spawn 独立 dsh web（带禁 stock ui-layout 的 overlay），不再复用浏览器/终端的 3080；',
  ...
  '仓库 desktop/scripts/acceptance.sh / .ps1 自动跑三条路径：A 复用（专用端口 3081 已有残留实例，退出不杀）、',
  ...
  '## 七、桌面端与会话的关系',
  '',
  '桌面壳有自己的 dsh web 实例（专用端口 3081，DSH_DESKTOP_PORT 可覆盖），与浏览器/终端 3080 互不干扰；',
  '数据仍共用 ~/.dsh（同 profile、同会话存储目录），历史对话/上下文完整保留；避免两边同时操作同一会话。',
