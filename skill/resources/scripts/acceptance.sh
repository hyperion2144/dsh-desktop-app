#!/usr/bin/env bash
# DeepSeek Harness Desktop Desktop · macOS 验收脚本（三条路径，源自实测验收清单）
# 用法：./scripts/acceptance.sh [binary] [port] [app_binary]
# 默认 binary=target/debug/dsh-desktop-tauriapp，port=3080，
# app_binary=src-tauri/target/release/bundle/macos/DeepSeek Harness Desktop Desktop.app/Contents/MacOS/dsh-desktop-tauriapp
set -euo pipefail
cd "$(dirname "$0")/.."

BIN="${1:-target/debug/dsh-desktop-tauriapp}"
PORT="${2:-3080}"
APP_BIN="${3:-src-tauri/target/release/bundle/macos/DeepSeek Harness Desktop Desktop.app/Contents/MacOS/dsh-desktop-tauriapp}"
...
if pgrep -q -f "dsh-desktop-tauriapp"; then
  ...
  echo "请先从托盘菜单退出DeepSeek Harness Desktop Desktop（Cmd+Q 只是隐藏），再运行本脚本。"
  exit 2
fi
