# DeepSeek Harness Desktop Desktop · Windows 构建环境脚本（实测：Win11 无管理员 + 国内网络）
# 用法：. .\scripts\build-env.ps1    （注意用点号加载，让环境变量进当前会话）
# 来源：docs/windows-build-notes.md §7 备忘清单

$ErrorActionPreference = "Stop"

Write-Host "[DeepSeek Harness Desktop Desktop] 配置构建环境..."
...
if (-not (Test-Path $configToml) -or -not (Select-String -Path $configToml -Pattern "rust-lld" -Quiet)) {
    Write-Host "[DeepSeek Harness Desktop Desktop] 提示：~/.cargo/config.toml 尚未固化无管理员工具链，"
    Write-Host "        请参照 scripts/cargo-config-no-admin.toml.example 合并配置。"
}

Write-Host "[DeepSeek Harness Desktop Desktop] 环境就绪："
