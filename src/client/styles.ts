...
/**
 * 桌面壳高级布局样式（照搬参考项目 dsh-plugin-desktop/advanced-shell，保持纯字符串自包含）。
 * 唯一差异：Electron 的 `-webkit-app-region: drag` 由 Tauri 的 `data-tauri-drag-region`
 * 属性取代（wry 不支持 webkit-app-region），因此参考项目里挂在伪元素上的拖拽区
 * 改为 AdvancedFrame 中真实渲染的 .dshDesktop*Drag 元素。
 */
